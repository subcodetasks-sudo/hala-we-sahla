  ...
  async function onSubmit(values: TrackOrderValues) {
    ...
    try {
      ...
      saveTrackOrderSession({
        request_number: data.request_number || values.request_number,
        phone: values.phone,
        data,
        fetchedAt: Date.now(),
      })
      ...
      const encoded = encodeURIComponent(
        data.request_number || values.request_number,
      )
      ...
      const message =
        getApiErrorMessages(error, {
          ...
          unknown: t("errors.trackFailed"),
        })[0] ?? t("errors.trackFailed")

      form.setError("root", { message })
    }
  }

  const isSubmitting = trackMutation.isPending || form.formState.isSubmitting

  return (
    <Card className="mx-auto max-w-lg gap-0 rounded-[28px] bg-card py-8 sm:rounded-[32px] sm:px-8 sm:py-10">
      ...
      >
        <FieldGroup className="gap-5">
          <Controller
            ...
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid || undefined}>
                <FieldLabel htmlFor="request_number" className="text-primary">
                ...
                <InputGroup className={inputGroupClassName} dir="ltr">
                  ...
                  <InputGroupInput
                    ...
                    onChange={(event) =>
                      field.onChange(keepRequestNumberInput(event.target.value))
                    }
                  />
                    ...
                    <InputGroupText className="gap-2 font-semibold text-foreground">
                      <span className="flex items-center gap-2 rounded-full bg-white px-2 py-1">
                        <ReactCountryFlag
                          ...
                          aria-hidden="true"
                        />
                        <span className="font-clash text-xs">
                          {SAUDI_COUNTRY_CODE}
                        </span>
                      </span>
                    </InputGroupText>
              ...
              render={({ field }) => (
                <label className="flex cursor-pointer items-center gap-2 text-sm font-medium text-foreground">
                  <Checkbox
                    ...
                    className="size-4 border-none bg-custom-gray"
                  />
          ...
          <Button
            ...
            className="mx-auto mt-1 h-14 w-fit justify-between gap-1.5 rounded-full bg-[#288296] text-base font-bold text-white hover:bg-[#288296]/90"
          >
            {isSubmitting ? (
