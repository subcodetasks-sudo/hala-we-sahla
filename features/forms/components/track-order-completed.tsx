...
function getDemoShippingNumber(requestNumber: string) {
  let hash = 0
  for (let i = 0; i < requestNumber.length; i += 1) {
    hash = (hash * 31 + requestNumber.charCodeAt(i)) >>> 0
  }
  const suffix = String(1000000 + (hash % 9000000)).slice(0, 7)
  return `SHD-${suffix}`
}
              ...
              <div className="flex h-full flex-col items-center rounded-[24px] bg-[#F5F5F5] p-4 sm:rounded-[32px]">
                ...
                <div className="mt-auto flex w-full flex-col gap-2">
                  <Button
                    ...
                    className="h-11 w-full gap-2 rounded-full bg-black text-sm font-semibold text-white hover:bg-black/90"
                  >
                    <CustomIcon
                      src="/icons/download.svg"
                      size={18}
                  ...
                  <Button
                    ...
                    className="h-11 w-full gap-2 rounded-full border-black/20 bg-white text-sm font-semibold text-black hover:bg-white hover:text-black"
                  >
                    <Eye className="size-4" aria-hidden="true" />
                    {t("invoice.view")}
                  </Button>
